...
void main() {
    ...
    for (int u = -1; u <= 1; ++u) {
        for (int v = -1; v <= 1; ++v) {
            ...
            float m = (t < 0.5) ? gain(t * 2., 6.5) : 1. - gain((t - 0.5) * 2., 6.5);
            m *= 0.5;
            ...
            cs = metaball(p - vec2(0.5) + m * dir, 0.1, p);
            cs = opOnion(cs, 0.05);
            cs = op_fill(cs, 0.);

            float cs2 = 0.;
            // cs2 = circle(p - vec2(0.5) + m * dir, 0.275);
            cs2 = metaball(p - vec2(0.5) + m * dir, 0.275, p);
            ...
            cs3 = metaball(vec2(0.5) + m * dir, 0.25, p);
            // cs3 = opOnion(cs3, 0.05);
            // cs3 = op_fill(cs3, 0.05);
