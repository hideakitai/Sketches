...
impl Light {
    pub fn new(
        ...
        let buffer = buffer::create_uniform_buffer(device, &raw);
        let bind_group_layout = Self::create_bind_group_layout(device);
        let bind_group =
            buffer::create_bind_group_from_buffers(device, &bind_group_layout, &[&buffer]);
    ...
    pub fn update(&mut self, queue: &wgpu::Queue, dt: Duration) {
        ...
        //         * old_position;
        self.raw.position = cgmath::Quaternion::from_axis_angle(
            (0.0, 1.0, 0.0).into(),
    ...
    'b: 'a,
{
    ...
    fn draw_light_mesh_instanced(
        ...
        println!("vertex_buffer    : {:?}", mesh.vertex_buffer);
        println!("index_buffer     : {:?}", mesh.index_buffer);
