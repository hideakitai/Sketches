use crate::renderer::draw::drawing::DrawingContext;
use crate::renderer::draw::primitive::Primitive;
use crate::renderer::draw::properties::spatial::{self, dimension, orientation, position};
use crate::renderer::draw::properties::{
    ColorScalar, LinSrgba, SetColor, SetDimensions, SetOrientation, SetPosition,
};
use crate::renderer::draw::{self, theme, Drawing};
use crate::text::{self, Align, Font, FontSize, Justify, Layout, Scalar, Wrap};
use nannou::geom::{self, Vector2};
use nannou::math::{cgmath, BaseFloat, Zero};
...
impl draw::renderer::RenderPrimitive for Text<f32> {
    fn render_primitive(
        ...
        let w = maybe_x
            .map(|s| <f32 as nannou::math::NumCast>::from(s).unwrap())
            .unwrap_or(200.0);
        let h = maybe_y
            .map(|s| <f32 as nannou::math::NumCast>::from(s).unwrap())
            .unwrap_or(200.0);
