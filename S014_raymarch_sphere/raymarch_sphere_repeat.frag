...
float sdSphereRepeat(vec3 p, float s, float d) {
    return sdSphere(mod(p, d) - d * 0.5, s);
}

vec3 normalSphereRepeat(vec3 p, float s, float d, float eps) {
    // normal is gradient of distance function
    return normalize(vec3(
        sdSphereRepeat(p + vec3(eps, 0., 0.), s, d) - sdSphereRepeat(p + vec3(-eps, 0., 0.), s, d),
        sdSphereRepeat(p + vec3(0., eps, 0.), s, d) - sdSphereRepeat(p + vec3(0., -eps, 0.), s, d),
        sdSphereRepeat(p + vec3(0., 0., eps), s, d) - sdSphereRepeat(p + vec3(0., 0., -eps), s, d)));
}
...
void main() {
    ...
    for (int i = 0; i < n_loop; ++i) {
        dist = sdSphereRepeat(ray_pos, sphere_size, sphere_repeat);
    ...
    if (abs(dist) < RAY_HIT_THRESH) {
        vec3 normal = normalSphereRepeat(ray_pos, sphere_size, sphere_repeat, EPS);
